...
def init_socket(local_receiver_port):
    try:
        ...
        exit()
    ss.bind(('localhost', local_receiver_port))
    return ss
...
def update_rto(sample_rtt, alpha=0.125, beta=0.25):
    global RTO
    estimated_rtt = (1 - alpha) * RTO + alpha * sample_rtt
    dev_rtt = (1 - beta) * RTO + beta * abs(sample_rtt - RTO)
    RTO = estimated_rtt + 4 * dev_rtt

def check_username(msg):
    if msg.msg.startswith('UN@'):
        username, remaining_msgs = msg.msg.split('UN@', 1)[1].split(' ', 1)
        ...
        return username, remaining_msgs
    return None, msg.msg
...
def receiver(ss, ooo_enabled):
    def get_msg_ack():
        ...
        if msg.msg.startswith('UN@'):
            username, remaining_msgs = check_username(msg)
    ...
    while last_acked <= last_seq_expected:
        ...
        start_time = time.time()  # Add this line to measure the start time
        msg, ack_msg, sender_addr = get_msg_ack()
        sample_rtt = time.time() - start_time  # Calculate the sample RTT
        update_rto(sample_rtt)  # Call the update_rto function with the sample RTT
