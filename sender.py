...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    while last_acked < final_ack:
        # Transmit a window of packets first
        first_to_tx = transmit_entire_window_from(win_left_edge)

        # If no new packets were transmitted, wait for ACKs
        if win_left_edge == first_to_tx:
            time.sleep(timeout)
            continue

        # Update unacked_packets list
        unacked_packets = [seq for seq in range(win_left_edge, first_to_tx)]

        # Wait for ACK using select()
        ready_to_read, _, _ = select.select([cs], [], [], timeout)
        ...
        if ready_to_read:
            ...
            # Update RTT and timeout value based on the ACK
            if ack_msg.ack in sample_rtt_dict:
                sample_rtt = time.time() - sample_rtt_dict[ack_msg.ack]
                estimated_rtt = (1 - alpha) * estimated_rtt + alpha * sample_rtt
                dev_rtt = (1 - beta) * dev_rtt + beta * abs(sample_rtt - estimated_rtt)
                timeout = estimated_rtt + 4 * dev_rtt

            # Update last_acked and slide the window if necessary
            if ack_msg.ack > last_acked:
                ...
                # Update unacked_packets list
                unacked_packets = [seq for seq in unacked_packets if seq >= last_acked]
                # Reset the duplicate ACK counter and last_received_ack
                duplicate_ack_counter = 0
                last_received_ack = None
            else:
                # Check for duplicate ACKs
                if last_received_ack == ack_msg.ack:
                    duplicate_ack_counter += 1
                else:
                    duplicate_ack_counter = 1
                    last_received_ack = ack_msg.ack

                # If three duplicate ACKs received, retransmit the oldest unacked packet
                if duplicate_ack_counter == 3:
                    oldest_unacked = unacked_packets[0]
                    index = seq_to_msgindex[oldest_unacked]
                    msg = messages[index]
                    m = Msg(oldest_unacked, __ACK_UNUSED, msg)
                    cs.sendto(m.serialize(), receiver_binding)
                    print("Retransmitted {} due to 3 duplicate ACKs".format(str(m)))
                    # Reset the duplicate ACK counter
                    duplicate_ack_counter = 0

        else:
            print("Timeout occurred, retransmitting packet")
            ...
            if unacked_packets:
                ...
                m = Msg(oldest_unacked, __ACK_UNUSED, msg)
                cs.sendto(m.serialize(), receiver_binding)
                print("Retransmitted {}".format(str(m)))
        
        if first_to_tx:
            sample_rtt_dict[first_to_tx] = time.time()
